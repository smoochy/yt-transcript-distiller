  ...
  (function() {
    ...
    function findTargetContainer() {
      const selectors = [
        ...
        // Old UI action menu fallback
        "#top-level-buttons-computed"
        // Old UI like/dislike row fallback
      ];
    ...
    async function scrapeTranscriptFromDOM() {
      ...
      const found = await waitForElement(segmentSelector, 1e4);
      if (!found) {
      ...
      const scrollContainer = document.querySelector('ytd-engagement-panel-section-list-renderer[visibility="ENGAGEMENT_PANEL_VISIBILITY_EXPANDED"] #content') || document.querySelector('ytd-engagement-panel-section-list-renderer[target-id*="transcript" i] #content') || document.querySelector(segmentSelector).closest("#content, #contents");
      ...
      for (let i = 0; i < 150; i++) {
        ...
        currentSegments.forEach((seg) => {
          ...
          if (textSpan) {
            ...
            const spans = Array.from(seg.querySelectorAll("span")).filter(
              (s) => !s.className.includes("Timestamp") && !s.className.includes("A11yLabel")
            );
    ...
    function applyThemeToUI(theme) {
      if (!theme) {
        const ytDark = document.documentElement.getAttribute("dark") !== null || document.querySelector("html[dark]") !== null || window.matchMedia("(prefers-color-scheme: dark)").matches;
    ...
    async function injectButton() {
      ...
      const targetContainer = await waitForElement("#owner #subscribe-button", 8e3);
      if (!targetContainer) {
    ...
    async function injectIntoTarget(targetContainer) {
      ...
      try {
        ...
        window.transcriptProtectionObserver.observe(targetContainer.parentNode, {
          childList: true,
          subtree: false
          // Changed from true to false to reduce overhead
        });
    ...
    async function getSettings() {
      function getStoragePromise(api) {
        return new Promise(async (resolve, reject) => {
          const timeoutId = setTimeout(() => reject(new Error("Storage operation timed out")), 500);
          try {
      ...
      chrome.storage.sync.get(["geminiApiKey", "distillerPrompt", "distillerLang", "invalidKey"], (r) => {
        ...
        if (hasKey && !keyInvalid) {
          ...
          if (keyInvalid) {
            ...
            warn.textContent = chrome.i18n.getMessage("err_key_invalid") || "\u26A0 API Key ung\xFCltig \u2013 bitte neu eingeben.";
...
${AMO_LINK}`,
      "fr": `condens\xE9 par Transcript Distiller
${AMO_LINK}`,
...
${err.message}`);
    ...
    async function resolveYouTubeData(videoUrl) {
      ...
      const domTitle = document.querySelector("#title h1")?.textContent?.trim() || document.querySelector("h1.ytd-watch-metadata")?.textContent?.trim();
      const title = domTitle || ytData?.videoDetails?.title || document.querySelector('meta[name="title"]')?.content || document.title.replace(" - YouTube", "") || "Unknown Title";
